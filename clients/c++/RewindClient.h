...
public:
    ...
    void popup(double x, double y, double r, uint32_t color, std::string text, size_t layer = DEFAULT_LAYER) {
        static const char *fmt =
            R"({"type": "popup", "x": %lf, "y": %lf, "r": %lf, "color": %u, "layer": %u, "text": %s})";
        send(format(fmt, x, y, r, color, layer, text.c_str()));
    }
